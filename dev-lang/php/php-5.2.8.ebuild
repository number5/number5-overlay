...
DEPEND="app-admin/php-toolkit fpm? ( >=dev-libs/libxml2-2.6.8 )"
...
pkg_setup() {
	...
	if use fpm && ! use cgi ; then
		eerror
		eerror "Enable CGI SAPI for use PHP-FPM patch"
		eerror
		ebeep 1
		die
	fi
	...
	for x in ${PHPSAPIS} ; do
		...
		case ${x} in
			...
			cgi)
				...
				# PHP fpm patch support
				if use fpm ; then
					my_conf="${my_conf} --enable-fpm --with-fpm-conf=/etc/php/cgi-php5/php-fpm.conf --with-fpm-log=/var/log/php-fpm/php-fpm.log --with-fpm-pid=/var/run/php-fpm.pid "
				fi
				phpconfutils_extension_enable "discard-path" "discard-path" 0
				phpconfutils_extension_enable "force-cgi-redirect" "force-cgi-redirect" 0
				php5_2-sapi_src_compile
		...
		case ${x} in
			...
			cgi)
				...
				if use fpm ; then
					...
					einfo "Installing PHP-FPM init.d-script"
...
pkg_postinst() {
	...
	if use cli || phpconfutils_usecheck cli ; then
		...
		if use fpm ; then
			...
			einfo "In current state patch support only static pm, not apache-like"
			einfo
		fi
	fi
